...
( function() {
    CKEDITOR.plugins.add( 'mets_enterkey', {
        init: function( editor ) {
            editor.addCommand( 'mets_enter', {
                modes: { wysiwyg: 1 },
                editorFocus: false,
                exec: function( editor ) {
                    console.log("mets_enter");
                    enter( editor );
                }
            } );

            editor.addCommand( 'mets_shiftEnter', {
                modes: { wysiwyg: 1 },
                editorFocus: false,
                exec: function( editor ) {
                    console.log("mets_shiftEnter");
                    shiftEnter( editor );
                }
            } );
            ...
            editor.setKeystroke( [
                [ 13, 'mets_enter' ],
                [ CKEDITOR.SHIFT + 13, 'mets_shiftEnter' ]
            ] );
        }
    } );
    ...
    CKEDITOR.plugins.mets_enterkey = {
        enterBlock: function( editor, mode, range, forceMode ) {
            ...
            if ( !isStartOfBlock && !isEndOfBlock ) {
                ...
                if ( nextBlock ){
                  //  var lineBreak=doc.createElement( 'span' );
                  //  lineBreak.setAttribute('class','lb');
                  //  lineBreak.setAttribute('data-lineid','L01');
                  //  lineBreak.append(doc.createText("\ufeff"));
                  //  nextBlock.append(lineBreak,true);
        ...
        enterBr: function( editor, mode, range, forceMode ) {
            ...
            if ( !forceMode && isEndOfBlock && headerTagRegex.test( startBlockTag ) ) {
                ...
                if ( ( newBlockDir = startBlock.getDirection() ) ) {
                    ...
                    if(startBlockTag=='p'){
                        lineBreak.setAttribute('class','lb_br');
                    }
                    lineBreak.insertAfter( startBlock );
                 //   var lb =doc.createElement("span");
                //    lb.setAttribute('class','lb');
                //    lb.setAttribute('data-lineid','L01');
                //    lb.append(doc.createText("\ufeff"));
                 //   lb.insertAfter( startBlock );
    ...
    var plugin = CKEDITOR.plugins.mets_enterkey,
